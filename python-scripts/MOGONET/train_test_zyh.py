...
from models_zyh import init_model_dict, init_optim
...
def sim_to_01(sim_list):
    # print(sim_list)
    # print(sim_list[0])
    # sim_list = sim_list[0]._values()
    # print(sim_list)
    a = np.array(sim_list._values())
    a2 = a.reshape((1,-1))
    a2 = np.sort(a2)
    len_x, len_y = a2.shape
    yuzhi = int(len_y*0.15)
    yuzhi_value = a2[:,-yuzhi]
    a[a >= yuzhi_value] = 1
    a[a < yuzhi_value] = 0
    a = a.tolist()
    return a
...
def gen_trte_adj_mat(data_tr_list, data_trte_list, trte_idx, adj_parameter):
    ...
    for i in range(len(data_tr_list)):
        ...
        kp = gen_adj_mat_tensor(data_tr_list[i], adj_parameter_adaptive, adj_metric)
        # kp1 = sim_to_01(kp)
        adj_train_list.append(kp)
        xp = gen_test_adj_mat_tensor(data_trte_list[i], trte_idx, adj_parameter_adaptive, adj_metric)
        # xp1 = sim_to_01(xp)
        adj_test_list.append(xp)
